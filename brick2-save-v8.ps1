# Brique 2 v8 - enregistre sous le VRAI nom attendu par Logos : Devis-1097.pdf
# (convention Devis-<devisId>.pdf). Gere le popup "remplacer ?" (clic Oui, Win32).
# Ouvre la fenetre PDF (clic PDF) PUIS lance ce script (attend 30 s). Aucune souris.
...
# >>> VRAI NOM ATTENDU PAR LOGOS <<<
$target = '\\PANO\wlogos2\Patients\LIENS\1720\Devis-1097.pdf'
...
# 1) Attendre la fenetre de sauvegarde
$hDlg=[IntPtr]::Zero
...
# 4) Gerer le popup "remplacer ?" -> cliquer Oui (id=6 IDYES, ou texte Oui)
    ...
    if ($t -match 'Confirmer' -or $t -match 'Enregistrer sous' -or $t -match 'remplacer') {
      foreach ($c in [W32]::Kids($h)) {
        if ([W32]::Cls($c) -eq 'Button' -and ([W32]::GetDlgCtrlID($c) -eq 6 -or [W32]::Txt($c) -match '^&?Oui')) {
          ...
          Say ("Popup 'remplacer' -> Oui clique (fenetre='$t').")
...
if (Test-Path $target) {
  $fi = Get-Item $target
  Say ("OK FICHIER PRESENT: $target  ($($fi.Length) octets, $($fi.LastWriteTime))")
} else { Say "PAS de fichier." }
