      ...
      try {
          ...
          if (uiCtx && uiCtx.honoraires > 0 && mmoCtx.patientsDir) {
            ...
            if (cands.length > 1) {
              ...
              for (const c of cands) {
                try {
                  const xml = mmoParser.readDevisXml(mmoPath, c.memoOff);
                  const meta = mmoParser.parseDevisXml(xml).metadata || {};
                  const hon = parseFloat(String(meta.honorairesG || '').replace(',', '.'));
                  if (!isNaN(hon)) {
        ...
        void (async () => {
          try {
            ...
            if (!done) {
              log('[PEC][bg] Aucun Devis-*.pdf ne correspond aux actes Logos — devis NON attaché (à vérifier)');
            }
          } catch (eBg) {
